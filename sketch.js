// 🐣 Eggzee — Cleaned GitHub Version (v10.2)
let state = "egg";
...
function setup() {
  ...
  eggzee = { visible: false, img: eggzeeAwakeImg, x: width / 2, y: height / 2, scale: 0.12, rotation: 0 };
...
function draw() {
  ...
  else if (state === "dance") {} // Animate handles this
...
function drawFeedScene() {
  ...
  for (let f of foods) {
    if (f.beingDragged) { f.x = mouseX; f.y = mouseY; }
    textSize(40); text(f.emoji,f.x,f.y);
  }
}
...
function drawSleepScene() {
  ...
  push(); translate(eggzee.x, eggzee.y + sin(frameCount*0.05)*8); image(eggzee.img,0,0,eggzee.img.width*0.1,eggzee.img.height*0.1); pop();
  fill(255); text("💤 Eggzee is sleeping... Tap to wake! 💫",width/2,height-100);
}
...
function drawButton(btn,emoji,label){
  fill(255,255,255,180); rect(btn.x-50,btn.y-40,100,80,20);
  fill(0); textSize(20); text(emoji,btn.x,btn.y-10); textSize(14); text(label,btn.x,btn.y+25);
}
...
function tellJoke(){ const jokes=["You crack me up every time 🥚😂","Keep calm and egg on 🧘‍♀️","This is so eggstroidinary! 🤩","I’m living sunny-side up ☀️"]; jokeText=random(jokes); showJoke=true; jokeTimer=millis(); }
