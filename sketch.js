// 🐣 Eggzee — Stable GitHub Patch (v12)
let state = "egg";
...
function setup() {
  ...
  eggzee = { visible: false, x: width/2, y: height/2, scale: 0.12, rotation: 0, img: eggzeeAwakeImg };
...
function drawEggScene() {
  ...
  fill(255); text("Tap the egg to hatch Eggzee 🥚",width/2,height-40);
...
function drawMiniGame(){
  ...
  for(let i=sparkles.length-1;i>=0;i--){
    const s=sparkles[i];
    noStroke(); fill(255,255,150,s.alpha);
...
function drawButton(btn,emoji,label){
  ...
  fill(0); textSize(20); text(emoji,btn.x,btn.y-10);
  textSize(14); text(label,btn.x,btn.y+25);
}
...
function mousePressed(){
  if(state==="egg"){ state="hatching"; crackTime=millis(); }
    ...
    hasWelcomed=true;
    if(insideButton(feedBtn)){ state="feed"; yumTimer=millis(); showYum=true; }
...
function tellJoke(){ const jokes=["You crack me up 🥚😂","Keep calm and egg on 🧘‍♀️","Eggstroidinary! 🤩","Sunny-side up ☀️"];
  jokeText=random(jokes); showJoke=true; jokeTimer=millis(); }
function drawFoods(){} function drawHearts(){} function drawYumBubble(){} function drawJoke(){} 
